...
export const KO_MESSAGES: MessageCatalog = {
  ...
  允许账号接入: "계정 접근 허용",
  停止账号接入: "계정 접근 중지",
  强制允许账号接入: "계정 접근 강제 허용",
  强制允许接入: "접근 강제 허용",
  账号已停止接入: "계정 접근이 중지되었습니다",
  账号已强制允许接入: "계정 접근이 강제로 허용되었습니다",
  "账号验证失败，保持停止接入: {error}":
    "계정 검증에 실패하여 접근 중지 상태를 유지합니다: {error}",
  当前无法确认账号状态: "현재 계정 상태를 확인할 수 없습니다",
  "账号验证成功，已允许接入": "계정 검증에 성공하여 접근을 허용했습니다",
  "开启允许接入时会先刷新验证账号；验证成功后才会启用，验证失败则保持停止接入。":
    "접근 허용을 켜면 먼저 새로 고침으로 계정을 검증합니다. 검증에 성공한 경우에만 활성화되며, 실패하면 접근 중지 상태를 유지합니다.",
  "账号已允许接入，但刷新失败: {error}":
