...
export const KO_API_KEYS_MESSAGES: MessageCatalog = {
  ...
  管理员视图: "관리자 보기",
  成员视图: "멤버 보기",
  "混合轮转（聚合 API 优先）": "하이브리드 로테이션(Aggregate API 우선)",
  "账号轮转只走账号池；聚合API轮转只走聚合API；账号优先混合轮转先完整尝试账号池，失败后使用聚合API；聚合API优先混合轮转先完整尝试聚合API，失败后使用账号池。":
    "계정 로테이션은 계정 풀만 사용하고 Aggregate API 로테이션은 Aggregate API만 사용합니다. 계정 우선 하이브리드는 계정 풀을 모두 시도한 뒤 Aggregate API로 전환하며, Aggregate API 우선 하이브리드는 그 반대 순서로 동작합니다.",
  "仅对会访问账号池的策略生效，可限制这把平台密钥只从指定账号计划类型中选择路由账号。":
    "계정 풀에 접근하는 전략에만 적용되며, 이 플랫폼 키가 선택한 계정 플랜 유형에서만 라우팅 계정을 고르도록 제한합니다.",
