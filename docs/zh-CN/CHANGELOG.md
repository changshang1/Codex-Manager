...
- 新增独立“商品池”页面：固定同步 PriceAI 的 ChatGPT Plus 试用订阅，支持全量手动同步、按标签定时同步、本地二次验证、提醒与变化记录、商家星标和“只看收藏商家”，并直接展示 PriceAI 返回的商家收录时间和公开运营时间。
- 管理员仪表盘新增“今日”和全局来源筛选，可按账号池、聚合 API 或具体来源任意多选；Token、请求、费用、趋势、模型统计和账号池剩余使用同一筛选口径。
- 平台 Key 新增“混合轮转（聚合 API 优先）”；模型 route 支持硬优先级、同级平滑权重和独立页面顺序号。

### Changed

- 商品池的定时同步和“立即同步全部”改为成功后原子替换全部报价；定时同步保留标签范围，手动同步始终拉取全部 Plus 商品，并保留收藏商家、提醒规则、变化记录和设置。
- 账号池增加“允许接入”快速开关，开启后立即刷新健康状态；列表默认先显示可用账号，账号池剩余只汇总筛选范围内真正可用的账号。
- 聚合 API 增加快速上移、下移和原子顺序保存；连接 `sort` 只控制页面显示，真实请求顺序改由模型 route 的 `priority` 和 `weight` 决定。
...
- 仪表盘和桌面项目启动器完成改版；新增网关/账号概览、今日 Token、缓存与推理用量、预计费用以及 5 小时/7 天剩余展示。
- 新增“启动时显示主界面”设置；关闭后可隐藏启动并从托盘重新打开。
- 用量趋势图新增模型多选、指标/占比切换、时间范围滑块、滚轮缩放和可访问交互。
...
- Codex 模型目录拆分为 OpenAI 官方目录和 CodexManager 受管目录；账号直连/账号池遵循官方目录，聚合与本地管理链路使用受管目录。
- 移除桌面/Web 的 `models_cache.json` 写入和下载入口，避免本地缓存覆盖官方或受管目录。

### Fixed

- Free 账号模型上限现在只过滤超过上限的 Free 账号候选，不会改写请求模型；Plus / Pro 账号不受影响，额度保护仍在过滤后执行，混合账号优先模式在账号耗尽后仍按现有策略回退到聚合 API。
- 强化账号故障转移和流式预检：有效内容开始前可识别额度耗尽并切换账号，输出开始后不再跨账号拼接响应；Responses WebSocket 会保留上下文并遵循环境代理。
- 修复用量 warmup cron 重复调度、账号池官方目录/profile 未及时同步、托盘 5 小时与 7 天重置标签颠倒，以及 Toast 遮挡页面操作等问题。
