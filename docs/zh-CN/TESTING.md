...
- “立即同步全部”忽略定时标签和页面筛选，抓取完整 Plus 分页，不验证、不通知，并原子替换报价。
- 自动同步按配置标签抓取并原子替换报价，验证低价前 20 个和可能命中提醒规则的报价。
- 成功空结果会清空报价；分页/字段/写入失败会整体回滚，不能留下半套新旧数据或发送通知。
- 替换会清理未返回及旧来源报价和对应提醒状态，同时保留同 ID 状态、收藏、规则、变化记录和设置。
