...
| 用量 | `service_usage_refresh` | `account/usage/refresh` | 参数可选：`accountId`、`markUnavailableOnFailure`；不传账号则刷新全部，快速开启账号时会要求失败落为不可用 |
| 仪表盘 | `service_dashboard_admin_usage_summary` | `dashboard/adminUsageSummary` | 参数：时间范围、序列粒度、`sourceKinds`、`selectedSources`、`includeUnavailableSources` |
| 仪表盘 | `service_dashboard_source_options` | `dashboard/sourceOptions` | 来源搜索、分页和已选项回显；支持账号与聚合 API 复合来源键 |
...
| 聚合 API | `service_aggregate_api_update_sorts` | `aggregateApi/updateSorts` | 参数：`updates[{apiId,sort}]`；在一个事务中原子更新页面显示顺序 |
...
| 商品池 | `service_marketplace_source_list` | `marketplace/sourceList` | 读取固定 Plus 来源及定时同步设置 |
| 商品池 | `service_marketplace_source_upsert` | `marketplace/sourceUpsert` | 参数：`payload`；兼容维护入口，当前页面不提供产品/来源编辑 |
| 商品池 | `service_marketplace_source_delete` | `marketplace/sourceDelete` | 参数：`id`；兼容维护入口 |
| 商品池 | `service_marketplace_offer_list` | `marketplace/offerList` | 参数：`payload{sourceConfigId,productId,limit}` |
| 商品池 | `service_marketplace_offer_verify` | `marketplace/offerVerify` | 参数：`offerKey`；只验证单个报价，不执行购买 |
| 商品池 | `service_marketplace_refresh` | `marketplace/refresh` | 手动全量同步全部 Plus；不验证、不通知 |
| 商品池 | `service_marketplace_change_list` | `marketplace/changeList` | 参数：`payload{limit}`；读取历史变化记录 |
| 商品池 | `service_marketplace_alert_list` | `marketplace/alertList` | 读取提醒规则 |
| 商品池 | `service_marketplace_alert_upsert` | `marketplace/alertUpsert` | 参数：`payload`；新增或更新提醒规则 |
| 商品池 | `service_marketplace_alert_delete` | `marketplace/alertDelete` | 参数：`id` |
| 商品池 | `service_marketplace_notification_get` | `marketplace/notificationGet` | 读取桌面通知开关 |
| 商品池 | `service_marketplace_notification_set` | `marketplace/notificationSet` | 参数：`enabled` |
| 商品池 | `service_marketplace_favorite_merchant_list` | `marketplace/favoriteMerchantList` | 读取已收藏商家 |
| 商品池 | `service_marketplace_favorite_merchant_set` | `marketplace/favoriteMerchantSet` | 参数：`payload{merchantKey,merchantName,favorite}`；设置或取消商家星标 |
...
| 仪表盘来源筛选 | `service_dashboard_source_options` / `dashboard/sourceOptions` |
| 仪表盘筛选后统计 | `service_dashboard_admin_usage_summary` / `dashboard/adminUsageSummary` |
| 商品池报价与设置 | `service_marketplace_*` / `marketplace/*` |
