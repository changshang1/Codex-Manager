...
- 当前语言目录下的 `CHANGELOG.md`：该语言版本历史与未发版更新的事实源；根目录 `CHANGELOG.md` 只负责多语言导航。
...
| 维护商品池、调整固定 PriceAI 范围和提醒规则 | [商品池维护说明](report/商品池维护说明.md) |
| 使用仪表盘来源筛选、账号快速启停和资源顺序 | [仪表盘来源筛选与资源状态说明](report/仪表盘来源筛选与资源状态说明.md) |
...
| [模型目录V2管理与计费说明.md](report/模型目录V2管理与计费说明.md) | 官方/受管目录分流、整数价格阶梯、route 优先级/权重/顺序号、instructions policy、本地 JSON 导入与钱包扣费 |
| [聚合API请求规则与配置说明.md](report/聚合API请求规则与配置说明.md) | 聚合上游连接、两种混合轮转、V2 route 调度、鉴权、余额查询与排障 |
| [仪表盘来源筛选与资源状态说明.md](report/仪表盘来源筛选与资源状态说明.md) | 账号/聚合 API 状态、页面顺序、仪表盘来源筛选与账号池剩余口径 |
